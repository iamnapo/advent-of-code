	...
	for (let row = 0; row < grid.length; row++) {
		for (let col = 0; col < grid[row]!.length; col++) {
			if (grid[row]![col] === "@") {
				...
				if (rollsAroundIt < 4) {
					accessibleRollCount += 1;
					rollsRemoved.push([row, col]);
				}
			}
		}
	}

	if (rollsRemoved.length === 0) break;

	while (rollsRemoved.length > 0) {
		const [row, col] = rollsRemoved.pop()!;
		grid[row!]![col!] = ".";
	}
}

console.log(`Day 4 - Part 2 solution is: ${accessibleRollCount}`);
